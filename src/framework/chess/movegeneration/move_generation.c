    ...
    if (popcount(checkers) > 1) {
        ...
        while (attacks) {
            ...
            if (!is_square_attacked_through_king(board, target_square, opponent_side, temp_occupancies)) {
                ...
                add_move(move_list,
                         encode_move(king_square, target_square, (side == white) ? K : k, 0, is_capture, 0, 0, 0));
            }
    ...
    for (int piece = (side == white ? P : p); piece <= (side == white ? K : k); piece++) {
        ...
        while (bitboard) {
            ...
            if (piece == P || piece == p) {
                ...
                while (attacks) {
                    target_square = bitscan_forward(attacks);
                    if (side == white && source_square >= a7 && source_square <= h7) {  // Promotions
                        add_move(move_list, encode_move(source_square, target_square, piece, Q, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, R, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, B, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, N, 1, 0, 0, 0));
                    } else if (side == black && source_square >= a2 && source_square <= h2) {
                        add_move(move_list, encode_move(source_square, target_square, piece, q, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, r, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, b, 1, 0, 0, 0));
                        add_move(move_list, encode_move(source_square, target_square, piece, n, 1, 0, 0, 0));
                ...
                if (board->enpassant != no_sq) {
                    ...
                    if (enpassant_attacks) {
                        int capture_square = (side == white) ? board->enpassant + 8 : board->enpassant - 8;
                ...
                while (attacks) {
                    ...
                    if (!is_square_attacked_through_king(board, target_square, opponent_side, temp_occupancies)) {
                        int is_capture = get_bit(board->occupancies[opponent_side], target_square) ? 1 : 0;
                        add_move(move_list, encode_move(king_square, target_square, (side == white) ? K : k, 0,
                                                        is_capture, 0, 0, 0));
                    }
                    pop_bit(&attacks, target_square);
                }
            ...
            } else {  // Knights, Bishops, Rooks, Queens
                if (piece == N || piece == n)
